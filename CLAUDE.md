...
- **"Claude's Further Studies" (per-module deck swap):** a module may also carry a `further: [...]` array of cards (same shape as `questions`) — deeper/harder cards. A small **✦ Further** toggle renders *beside that module* in the picker (a `.further-toggle` button inside a `.module-row` flex wrapper; state in the in-memory `furtherModules` Set, default off). When on, `buildQuestions` **swaps the deck** for that module — you study *only* the `further` cards instead of the basic `questions`, not both. Turning it on also auto-selects the module; the picker count badge flips to the active deck's size (e.g. 9 → 8). The cards share the module's `id`, so `cardId` stays stable and mastery tracks for free; keep each `explain` distinct so ids don't collide with the base cards. `further` cards are **excluded** from the "Mastered"/"Cards" totals (`allCards`/`masteryStats` iterate only `m.questions`). Currently only the **Aggregate Functions** module uses this (8 deeper cards on GROUP BY/HAVING, NULL handling, clause order).
